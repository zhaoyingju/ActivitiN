...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		...
		{
			this.primary = Collections.synchronizedMap(new LinkedHashMapAnonymousInnerClassHelper(this, size, secondary));
			this.secondary = secondary == null ? null : Collections.synchronizedMap(secondary);
		}

		private class LinkedHashMapAnonymousInnerClassHelper : LinkedHashMap<string, Tree>
		{
			...
			public LinkedHashMapAnonymousInnerClassHelper(Cache outerInstance, int size, IDictionary<string, Tree> secondary) : base(16, 0.75f, true)
			{
			...
			protected internal override bool removeEldestEntry(KeyValuePair<string, Tree> eldest)
			{
				if (size() > size)
				{
					if (outerInstance.secondary != null) // move to secondary cache
					{
