...
namespace org.activiti.engine.impl.juel
{
	...
	public class NumberOperations
	{
		...
		public static Number neg(TypeConverter converter, object value)
		{
			...
			if (value is double?)
			{
				return Convert.ToDouble(-(double)((double?)value));
			}
			if (value is float?)
			{
				return Convert.ToSingle(-(float)((float?)value));
			}
			if (value is string)
			{
				if (isDotEe((string)value))
				{
					return Convert.ToDouble(-(double)converter.convert(value, typeof(double?)));
				}
				return Convert.ToInt64(-(long)converter.convert(value, typeof(long?)));
			}
			if (value is long?)
			{
				return Convert.ToInt64(-(long)((long?)value));
			}
			if (value is int?)
			{
				return Convert.ToInt32(-(int)((int?)value));
			}
			if (value is short?)
			{
				return Convert.ToInt16((short)-(short)((short?)value));
			}
			if (value is sbyte?)
			{
				return Convert.ToByte((sbyte)-(sbyte)((sbyte?)value));
			}
