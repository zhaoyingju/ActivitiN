...
namespace org.activiti.engine.impl.calendar
{
	...
	public class CronExpression : ICloneable
	{
	  ...
	  public static readonly int MAX_YEAR = new DateTime().get(DateTime.YEAR) + 100;
...
//ORIGINAL LINE: public CronExpression(String cronExpression, org.activiti.engine.runtime.ClockReader clockReader, java.util.TimeZone timeZone) throws java.text.ParseException
	  public CronExpression(string cronExpression, ClockReader clockReader, TimeZone timeZone) : this(cronExpression, clockReader)
	  {
	  ...
	  ///           <CODE>CronExpression</CODE> </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public CronExpression(String cronExpression, org.activiti.engine.runtime.ClockReader clockReader) throws java.text.ParseException
...
//ORIGINAL LINE: protected int storeExpressionVals(int pos, String s, int type) throws java.text.ParseException
	  protected internal virtual int storeExpressionVals(int pos, string s, int type)
	  {
		...
		if (c == '?')
		{
		  ...
		  {
			int val = (int)((int?) daysOfMonth.Max);
			if (val == NO_SPEC_INT)
			{
			  throw new ParseException("'?' can only be specfied for Day-of-Month -OR- Day-of-Week.", i);
			}
		...
		{
		  int val = Convert.ToInt32(Convert.ToString(c));
		  i++;
		  if (i >= s.Length)
		  {
...
//ORIGINAL LINE: protected int checkNext(int pos, String s, int val, int type) throws java.text.ParseException
	  protected internal virtual int checkNext(int pos, string s, int val, int type)
	  {
		...
		if (c == '-')
		{
		  i++;
		  c = s[i];
		  int v = Convert.ToInt32(Convert.ToString(c));
		  end = v;
		  i++;
		  if (i >= s.Length)
		  {
		  ...
		  if (i < s.Length && ((c = s[i]) == '/'))
		  {
			i++;
			c = s[i];
			int v2 = Convert.ToInt32(Convert.ToString(c));
			i++;
			if (i >= s.Length)
			{
...
//ORIGINAL LINE: protected void addToSet(int val, int end, int incr, int type) throws java.text.ParseException
	  ...
	  {
		char c = s[i];
		StringBuilder s1 = new StringBuilder(Convert.ToString(v));
		while (c >= '0' && c <= '9')
		{
		  s1.Append(c);
		  i++;
		...
		val.value = Convert.ToInt32(s1.ToString());
		return val;
	  }
	  ...
	  {
		DateTime date = new GregorianCalendar();
		date = new DateTime(afterTime);

		DateTime after = getTimeAfter(date);

		return after == null ? null : after.Ticks;
	  }
	  ...
	  public virtual DateTime getTimeAfter(DateTime afterTime)
	  {
		...
		DateTime cl = clockReader.getCurrentCalendar(TimeZone); //new java.util.GregorianCalendar(getTimeZone());

		// move ahead one second, since we're computing the time *after* the
		// given time
		afterTime.AddSeconds(1);
		// CronTrigger does not deal with milliseconds
		cl.TimeInMillis = afterTime.TimeInMillis;
		...
		while (!gotOne)
		{
		  ...
		  if (st != null && !st.Empty)
		  {
			sec = (int)((int?) st.first());
		  }
		  else
		  {
			sec = (int)((int?) seconds.Min);
			min++;
		  ...
		  min = cl.Minute;
		  int hr = cl.get(DateTime.HOUR_OF_DAY);
		  t = -1;
		  ...
		  {
			t = min;
			min = (int)((int?) st.first());
		  }
		  else
		  {
			min = (int)((int?) minutes.Min);
			hr++;
		  }
		  if (min != t)
		  {
		  ...
		  hr = cl.get(DateTime.HOUR_OF_DAY);
		  int day = cl.Day;
		  t = -1;
		  ...
		  if (dayOfMSpec && !dayOfWSpec) // get day by day of month rule
		  {
			...
			else if (st != null && !st.Empty)
			{
			  t = day;
			  day = (int)((int?) st.first());
		  ...
		  else if (dayOfWSpec && !dayOfMSpec)
		  {
			...
			{
			  // the month?
			  int dow = (int)((int?) daysOfWeek.Min); // desired
			  // d-o-w
		  ...
		  {
			t = mon;
			mon = (int)((int?) st.first());
		  }
		  else
		  {
			mon = (int)((int?) months.Min);
			year++;
		  }
		  if (mon != t)
		  {
	  ...
	  protected internal virtual void setCalendarHour(DateTime cal, int hour)
	  {
		...
		if (cal.get(DateTime.HOUR_OF_DAY) != hour && hour != 24)
		{
