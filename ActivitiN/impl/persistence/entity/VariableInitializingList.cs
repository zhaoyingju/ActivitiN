...
namespace org.activiti.engine.impl.persistence.entity
{
	...
	/// </summary>
	public class VariableInitializingList : List<VariableInstanceEntity>
	{

	  private const long serialVersionUID = 1L;

	  public override void add(int index, VariableInstanceEntity e)
	  {
		base.Insert(index, e);
		initializeVariable(e);
	  }

	  public override bool add(VariableInstanceEntity e)
	  {
		initializeVariable(e);
		return base.Add(e);
	  }
	  public override bool addAll<T1>(ICollection<T1> c) where T1 : VariableInstanceEntity
	  {
		foreach (VariableInstanceEntity e in c)
		{
		  initializeVariable(e);
		}
		return base.AddRange(c);
	  }
	  public override bool addAll<T1>(int index, ICollection<T1> c) where T1 : VariableInstanceEntity
	  {
		foreach (VariableInstanceEntity e in c)
		{
		  initializeVariable(e);
	  ...
	  /// <summary>
	  /// If the passed <seealso cref="VariableInstanceEntity"/> is a binary variable and the command-context is active,
	  /// the variable value is fetched to ensure the byte-array is populated.
	  /// </summary>
	  protected internal virtual void initializeVariable(VariableInstanceEntity e)
	  {
		if (Context.CommandContext != null && e != null && e.Type != null)
		{
		  ...
		  {
			((CacheableVariable) e.Type).ForceCacheable = true;
		  }
		}
	  }
	}
