...
//ORIGINAL LINE: @SuppressWarnings("unchecked") @Override protected void eventNotificationsCompleted(InterpretableExecution execution)
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getCanonicalName method:
		...
		else if (execution.Scope)
		{
		  ...
		  if (parentActivityBehavior is CompositeActivityBehavior)
		  {
			...
			if (activity.Scope && activity.OutgoingTransitions.Count == 0)
			{
		  ...
		  else
		  {
			...
			if (activity.Parent == activity.ProcessDefinition)
			{
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("unchecked") protected boolean isExecutionAloneInParent(InterpretableExecution execution)
	  protected internal virtual bool isExecutionAloneInParent(InterpretableExecution execution)
	  {
		ScopeImpl parentScope = (ScopeImpl) execution.Activity.Parent;
