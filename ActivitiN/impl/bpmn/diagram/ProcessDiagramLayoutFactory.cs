...
namespace org.activiti.engine.impl.bpmn.diagram
{
	...
	using DiagramElement = org.activiti.engine.repository.DiagramElement;
	using DiagramLayout = org.activiti.engine.repository.DiagramLayout;
	using DiagramNode = org.activiti.engine.repository.DiagramNode;
	...
	public class ProcessDiagramLayoutFactory
	{
	  ...
	  public virtual DiagramLayout getProcessDiagramLayout(InputStream bpmnXmlStream, InputStream imageStream)
	  {
	  ...
	  /// <returns> Layout of the process diagram </returns>
	  /// <returns> null when parameter imageStream is null </returns>
	  public virtual DiagramLayout getBpmnProcessDiagramLayout(Document bpmnModel, InputStream imageStream)
	  {
		if (imageStream == null)
		{
		  return null;
...
//JAVA TO C# CONVERTER TODO TASK: There is no .NET Dictionary equivalent to the Java 'putAll' method:
	  ...
	  protected internal virtual Document parseXml(InputStream bpmnXmlStream)
	  {
	  ...
	  protected internal virtual DiagramNode getDiagramBoundsFromImage(InputStream imageStream)
	  {
		return getDiagramBoundsFromImage(imageStream, 0, 0);
	  }

	  protected internal virtual DiagramNode getDiagramBoundsFromImage(InputStream imageStream, int offsetTop, int offsetBottom)
	  {
		BufferedImage image;
		try
		{
	  ...
	  protected internal virtual IDictionary<string, DiagramElement> transformBoundsForImage(DiagramNode diagramBoundsImage, DiagramNode diagramBoundsXml, IDictionary<string, DiagramNode> listOfBounds)
	  {
		...
		foreach (KeyValuePair<string, DiagramNode> bounds in listOfBounds)
		{
	  ...
	  protected internal virtual DiagramNode transformBoundsForImage(DiagramNode diagramBoundsImage, DiagramNode diagramBoundsXml, DiagramNode elementBounds)
	  {
		...
		elementBoundsForImage.X = (double) Math.Round((elementBounds.X - diagramBoundsXml.X) * scalingFactorX + diagramBoundsImage.X);
		elementBoundsForImage.Y = (double) Math.Round((elementBounds.Y - diagramBoundsXml.Y) * scalingFactorY + diagramBoundsImage.Y);
		elementBoundsForImage.Width = (double) Math.Round(elementBounds.Width * scalingFactorX);
		elementBoundsForImage.Height = (double) Math.Round(elementBounds.Height * scalingFactorY);
	  ...
	  protected internal virtual IDictionary<string, DiagramNode> fixFlowNodePositionsIfModelFromAdonis(Document bpmnModel, IDictionary<string, DiagramNode> elementBoundsFromBpmnDi)
	  {
		if (isExportedFromAdonis50(bpmnModel))
		{
		  ...
		  foreach (KeyValuePair<string, DiagramNode> entry in elementBoundsFromBpmnDi)
		  {
