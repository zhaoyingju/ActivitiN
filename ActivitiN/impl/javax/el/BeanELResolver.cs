...
//ORIGINAL LINE: final java.beans.PropertyDescriptor[] properties;
				...
				{
					return Collections.emptyList<FeatureDescriptor> ().GetEnumerator();
				}
				return new IteratorAnonymousInnerClassHelper(this, properties);
			}
			return null;
		}
		...
		private class IteratorAnonymousInnerClassHelper : IEnumerator<FeatureDescriptor>
		{
			...
			public IteratorAnonymousInnerClassHelper(BeanELResolver outerInstance, PropertyDescriptor[] properties)
			{
		...
		public override object getValue(ELContext context, object @base, object property)
		{
			...
			if (isResolvable(@base))
			{
				Method method = toBeanProperty(@base, property).ReadMethod;
				if (method == null)
				{
				...
				catch (InvocationTargetException e)
				{
					...
					throw new ELException(e.TargetException);
				}
				catch (Exception e)
				{
					throw new ELException(e);
		...
		public override void setValue(ELContext context, object @base, object property, object value)
		{
			...
			if (isResolvable(@base))
			{
				...
				Method method = toBeanProperty(@base, property).WriteMethod;
				if (method == null)
				{
				...
				catch (InvocationTargetException e)
				{
					// for an invocation target exception, the causing exception is not the Cause()
					// but the target exception
					throw new ELException("Cannot write property: " + property, e.TargetException);
				}
	  ...
	  private Method findMethod(object @base, string name, Type[] types, int paramCount)
	  {
			if (types != null)
			{
				try
			...
			foreach (Method method in @base.GetType().GetMethods())
			{
		...
		private object[] coerceParams(ExpressionFactory factory, Method method, object[] @params)
		{
			...
			if (method.VarArgs)
			{
				...
				if (length == 1)
				{
					...
					{
						if (types[varargIndex].IsInstanceOfType(source)) // use source array as is
						{
							array = source;
