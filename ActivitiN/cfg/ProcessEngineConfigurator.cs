...
namespace org.activiti.engine.cfg
{
	...
	/// <summary>
	/// Implementations of this class can be plugged into a <seealso cref="ProcessEngineConfigurationImpl"/>.
	...
	public interface ProcessEngineConfigurator
	{
		...
		/// 
		/// Allows to tweak the process engine by passing the <seealso cref="ProcessEngineConfigurationImpl"/>
		/// which allows tweaking it programmatically.
		/// 
		/// An example is the jdbc url. When a <seealso cref="ProcessEngineConfigurator"/> instance
	  ...
	  /// No default properties need to be overridden for this (otherwise the <seealso cref="#beforeInit(ProcessEngineConfigurationImpl)"/> 
	  /// method should be used) so the logic contained in this method is executed
	  /// after initialisation of the default objects.
	  /// 
	  /// Probably a better name would be 'afterInit' (cfr <seealso cref="#beforeInit(ProcessEngineConfigurationImpl)"/>),
	  /// but not possible due to backwards compatibility.
	  /// </summary>
	  void configure(ProcessEngineConfigurationImpl processEngineConfiguration);

	  /// <summary>
	  /// When the <seealso cref="ProcessEngineConfigurator"/> instances are used, they are first
	  /// ordered by this priority number (lowest to highest).
	  /// If you have dependencies between <seealso cref="ProcessEngineConfigurator"/>
